...
| 수평 갤러리 | 사진 수 기반 가변 높이, 이전 구간과 `-140svh` 중첩 | `100svh` | 수평 이동 완료 뒤 안정 viewport 2개 길이만큼 마지막 사진을 고정한 후 초대장 전환 |
...
개인화 링크 생성 화면은 `/invite-builder`입니다. 관리자 코드를 포함해 `POST /api/invitations`를 호출하면 이름·문구가 Notion 데이터 소스에 저장되고, `/#i=<짧은 ID>` URL이 생성됩니다. 초대장에서는 `POST /api/invitations/resolve`로 활성 상태와 만료일을 확인해 문구를 가져옵니다. 운영 환경에는 `NOTION_DATA_SOURCE_ID`, `NOTION_ACCESS_TOKEN`, `INVITATION_BUILDER_SECRET`이 필수입니다. Notion 행의 `활성`을 해제하면 링크 하나만 취소할 수 있습니다. 배포 이전에 생성한 `#invite=` 링크는 `INVITATION_TOKEN_SECRET`을 유지하는 동안 계속 호환됩니다.
